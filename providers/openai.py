...
from ._openai_wire import OpenAIWireMixin
...
class OpenAIProvider(OpenAIWireMixin):
    def __init__(
        self,
        api_key: str,
        model: str,
    ...
    @staticmethod
    def _usage_from_sdk(u: Any) -> Usage:
        """Build Usage from the SDK's typed usage object.

        ``prompt_tokens_details.cached_tokens`` is the standard path; llama-server
        puts ``cached_tokens`` directly on the usage object, so fall back to that.
        """
        if u is None:
            return Usage()
    ...
    async def complete(
        ...
        # Content + thinking-block detection (reasoning models emit <think>...</think>)
        ...
        # Tool calls — arguments arrive as JSON strings
        tool_calls: list[ToolCall] = []
        for tc in (message.tool_calls or []):
            args_raw = tc.function.arguments
            parsed = _parse_json(args_raw) if isinstance(args_raw, str) else args_raw
            args = parsed if isinstance(parsed, dict) else {"raw": args_raw}
            tool_calls.append(ToolCall(id=tc.id, name=tc.function.name, arguments=args))

        # Normalize finish_reason to our stop_reason vocabulary
        stop = _STOP_REASON_MAP.get(choice.finish_reason or "", "end_turn")

        usage = self._usage_from_sdk(response.usage)
        if usage.cache_read_tokens == 0 and usage.input_tokens > 1000:
            log.debug(
    ...
    async def stream(
        ...
        async for item in threaded_stream(_stream_factory):
            if not item.choices and item.usage:
                ...
                yield StreamDelta(usage=self._usage_from_sdk(item.usage))
                continue
