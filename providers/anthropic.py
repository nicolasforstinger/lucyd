...
class AnthropicProvider:
    ...
    @classmethod
    def _response_to_llm(cls, response: Any) -> LLMResponse:
        """Convert an Anthropic SDK Message (or final-message object) to LLMResponse."""
        text_parts: list[str] = []
        ...
        stop = _STOP_REASON_MAP.get(response.stop_reason, "end_turn")

        return LLMResponse(
            text="\n".join(text_parts) if text_parts else None,
            tool_calls=tool_calls,
            stop_reason=stop,
            usage=cls._usage_from_sdk(response.usage),
    ...
    async def stream(
        ...
        async for item in threaded_stream(_stream_factory):
            ...
            if hasattr(item, "usage") and hasattr(item, "stop_reason"):
                yield StreamDelta(
                    stop_reason=_STOP_REASON_MAP.get(item.stop_reason, "end_turn"),
                    usage=self._usage_from_sdk(item.usage),
                )
                break
