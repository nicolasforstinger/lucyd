...
**Never assume a doc claim is correct because it looks reasonable.** "13 agent tools" looked reasonable. It was wrong — there were 16. "thinking_mode = enabled" looked reasonable. It was wrong — the valid values are "adaptive", "budgeted", or "disabled." Every claim gets traced to source, no matter how plausible it seems. The claims that look most correct are the ones most likely to survive unverified.
...
### P-031: Context budget documentation
Verify the default `lucyd.toml.example` documents the relationship between system prompt size and model context window. For small-context models (4k-8k), the system prompt can silently consume the entire context window. Operators need:
1. A comment explaining that workspace files + skills + tools + recall all contribute to context usage
2. Guidance on `max_context_tokens` setting relative to workspace size
3. If a startup warning mechanism exists, documentation of what triggers it

### P-032: Default documentation for tunable parameters
```bash
grep -B2 -A2 'priority\|threshold\|budget\|limit\|timeout\|window' lucyd.toml.example
```
For each tunable behavior parameter in `lucyd.toml.example`: does it have a comment explaining what it controls, why the default was chosen, and when an operator should change it? Defaults without context are invisible tradeoffs. Particular attention to parameters whose defaults only make sense for a specific deployment type (companion, business, local, cloud).
