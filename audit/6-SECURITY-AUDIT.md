...
### P-028: Control Endpoint Audit
For each HTTP endpoint that triggers a state change (POST/PUT/DELETE), trace the mutation path:
1. Does it go through the queue? (AI-002 compliant)
2. If not, can a concurrent message be mid-processing when the endpoint fires?
3. If it races, what state can be corrupted?

```bash
# Find all state-mutating HTTP handlers
grep -rn 'def handle_' channels/http_api.py | grep -v test
# For each, trace whether it enqueues or mutates directly
grep -A20 'def handle_' channels/http_api.py | grep -E 'queue\.put\|session\.|_save|\.close|\.reset|\.write|memory'
```

This extends the existing input→capability→boundary model to cover internal mutation paths, not just external attacker paths. A race condition between an HTTP endpoint and `_process_message()` is a concurrency vulnerability even if the endpoint is authenticated.
...
- Is there a deny-list on sub-agent tools? (`_DEFAULT_SUBAGENT_DENY` / `_subagent_deny`)
...
**Use `9-BUG-FIX-WORKFLOW.md` for the actual fix process.** This phase identifies WHAT to fix. The bug fix workflow defines HOW to fix it.
