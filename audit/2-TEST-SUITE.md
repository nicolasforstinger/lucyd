...
- **Source bug:** Production code changed and test correctly caught it. Fix the source. Use audit/9-BUG-FIX-WORKFLOW.md.
...
### 3f: Test Rot Detection

**Why:** Tests decay silently. A function gets renamed, a module gets deleted, a class gets refactored — but the test referencing the old name still passes because it was testing a mock, or it was skipped, or its assertion was trivially true. These tests provide false confidence: they exist in the count but verify nothing.

**Tests referencing removed code:**
```bash
# Find all imports in test files and check they resolve
for f in tests/test_*.py; do
  grep -oP 'from [a-z_.]+ import \K[A-Za-z_, ]+' "$f" | tr ',' '\n' | tr -d ' ' | while read name; do
    [ -z "$name" ] && continue
    grep -rq "def $name\|class $name\|^$name " *.py tools/*.py channels/*.py providers/*.py 2>/dev/null || \
      echo "STALE IMPORT: $name in $f"
  done
done
```
A test importing a function that no longer exists is dead weight. If the import doesn't error (because it's conditional or the test is skipped), the test silently contributes nothing.

**Permanently skipped tests:**
```bash
# Find all skipped tests and their reasons
grep -rn '@pytest.mark.skip\|pytest.skip(' tests/ | grep -v __pycache__
```
For each: is the skip reason still valid? If the blocker was resolved (bug fixed, feature implemented, dependency updated), unskip the test. If the skip has no reason comment, that's a finding — skips without rationale accumulate forever.
...
for node in ast.walk(tree):
    if isinstance(node, (ast.FunctionDef, ast.AsyncFunctionDef)) and node.name.startswith('test_'):
        ...
        if 'assert' not in body_src.lower() and 'Assert' not in body_src and 'raises' not in body_src and 'warn' not in body_src:
            print(f'NO ASSERTION: {node.name} in $f (line {node.lineno})')
" 2>/dev/null
done

# Tests that assert True or pass (trivially passing)
grep -rn 'assert True\|assert 1\|pass$' tests/ | grep 'def test_' -A2 | grep -v __pycache__
```
A test without assertions is a test that verifies nothing — it only proves the code doesn't crash. A test asserting `True` is worse — it pretends to verify something.

**Unused test helpers:**
```bash
# Find functions in test files that aren't test functions and aren't fixtures
for f in tests/test_*.py; do
  grep -oP 'def \K[a-z_]+(?=\()' "$f" | grep -v '^test_' | while read helper; do
    # Check if it's used anywhere in test files
    count=$(grep -rn "$helper" tests/ | grep -v "def $helper" | grep -v __pycache__ | wc -l)
    [ "$count" -eq 0 ] && echo "UNUSED HELPER: $helper() in $f"
  done
done
```
Dead helpers in test files are dead code. They clutter the test infrastructure and may be misleading (suggesting test patterns that aren't actually used).
