...
### P-027: cost.db Producer Completeness
For each LLM call site identified in P-027 (Stage 1 grep), verify it appears as a producer in the cost.db pipeline. The producer list must match the call site list exactly. Any call site missing from the producer list is an untracked cost.
```bash
# Cross-reference: all provider.complete() call sites
grep -rn 'provider\.complete\|\.complete(' --include='*.py' | grep -v test | grep -v __pycache__
# vs all _record_cost() call sites
grep -rn '_record_cost' --include='*.py' | grep -v test | grep -v __pycache__
```
Every `complete()` call site must have a corresponding `_record_cost()` call in its success path. This includes compaction, consolidation, synthesis, and any future LLM call paths.
...
| Evolution output | `workspace/MEMORY.md`, `workspace/USER.md` | Daemon agentic loop (triggered by `lucyd-send --evolve` cron) |
...
| Memory evolution (`lucyd-send --evolve`) | cron | `20 4 * * *` | `evolution_state` table + MEMORY.md/USER.md | |
...
SELECT summary, date FROM episodes ORDER BY date DESC LIMIT 1;
-- Threshold: 48 hours (depends on conversation activity).

-- Structured memory: commitments
SELECT what, created_at FROM commitments WHERE status = 'open' ORDER BY created_at DESC LIMIT 1;
...
| Structured memory: episodes | `extract_episode()` -> `search_episodes()` returns episode |
| Structured memory: commitments | `extract_facts()` (commitment type) -> `get_open_commitments()` returns commitment |
...
## Phase 4b: Dependency Hygiene

**Why:** The dependency chain isn't just internal data pipelines — it includes external packages. An installed package that nothing imports is dead weight (attack surface, maintenance burden, install time). A deprecated package is a ticking clock. Duplicate packages providing the same functionality create confusion and inconsistency.

### Installed But Never Imported

```bash
# List all installed packages (excluding stdlib and dev tools)
pip list --format=columns | tail -n +3 | awk '{print $1}' | tr '[:upper:]' '[:lower:]' | sort > /tmp/installed_pkgs.txt

# List all imported packages in production code
grep -rhn 'import \|from ' *.py tools/*.py channels/*.py providers/*.py | \
  grep -v test | grep -v __pycache__ | \
  sed 's/from //;s/ import.*//;s/import //;s/\..*//' | \
  tr '[:upper:]' '[:lower:]' | sort -u > /tmp/imported_pkgs.txt

# Find installed but never imported
# Note: package names don't always match import names (e.g., Pillow→PIL, python-dotenv→dotenv)
# Manual verification needed for each result
comm -23 /tmp/installed_pkgs.txt /tmp/imported_pkgs.txt
```

For each result: is this package imported under a different name? (e.g., `PyYAML` → `import yaml`, `python-dotenv` → `import dotenv`). If genuinely unused, it's a finding — the package should be removed from requirements. Dev-only tools (pytest, mutmut, ruff, mypy) are exempt.
...
For each outdated package: is the update a patch (safe), minor (review changelog), or major (breaking changes possible)? Packages with no release in 2+ years should be investigated — are they stable/complete (acceptable) or abandoned (find replacement)?

### Duplicate Functionality

Check for packages that overlap in purpose:
- Multiple HTTP clients (`httpx` + `requests` + `aiohttp`) — is each one used for a distinct purpose?
- Multiple JSON libraries (`json` + `orjson` + `ujson`) — are multiple needed?
- Multiple testing utilities beyond pytest — are they all active?

```bash
# HTTP clients
grep -rn 'import httpx\|import requests\|import aiohttp\|import urllib' --include='*.py' | grep -v test | grep -v __pycache__ | grep -v .venv
```

Duplicate packages aren't automatically bad — `httpx` for sync and `aiohttp` for async is valid. But document the justification. If two packages serve the same purpose and one could be removed, that's a finding.
