...
```
For each call site: trace forward. Is `_record_cost()` called with the usage data? If not, the cost is invisible to the operator.
...
```
For each: does the truncated output include a signal (marker text, metadata flag) that tells the consumer it was truncated? Silent truncation is a finding.

### P-030: Log statement without trace_id
```bash
# Verify all log calls in the message processing path include trace_id
...
### P-032: Architectural defaults without documentation
```bash
...
```bash
# TODO/FIXME/HACK/WORKAROUND/XXX comments — technical debt markers
grep -rn 'TODO\|FIXME\|HACK\|WORKAROUND\|XXX' --include='*.py' | grep -v test | grep -v __pycache__ | grep -v .venv | grep -v mutants

# Monkey patches — runtime modification of imported modules
grep -rn 'monkey\|__class__\s*=\|setattr.*import\|types\.MethodType' --include='*.py' | grep -v test | grep -v __pycache__ | grep -v .venv

# Bypassed abstractions — direct attribute access on objects that have a public API
# (Look for _private attribute access across module boundaries)
grep -rn '\.\(_[a-z]' --include='*.py' | grep -v 'self\.\|cls\.' | grep -v test | grep -v __pycache__ | grep -v .venv | head -30
```
For each TODO/FIXME/HACK: is it still relevant? If the issue was fixed, remove the comment. If the hack is still needed, is there a cleaner approach now? Permanent hacks that work around framework limitations should be documented as Known Gaps or converted to proper solutions.

For each monkey patch or bypassed abstraction: why does this exist? If the public API is insufficient, fix the API. If it's a test helper leaking into production, extract it properly.

### Structural Issues
```bash
# Circular imports — try importing each module and check for ImportError
for f in $(find . -name "*.py" -not -path "./.venv/*" -not -path "./tests/*" -not -path "./mutants/*" -not -path "./__pycache__/*"); do
  python -c "import importlib.util; spec = importlib.util.spec_from_file_location('mod', '$f'); mod = importlib.util.module_from_spec(spec)" 2>&1 | grep -i "circular\|import"
done

# God files — production files over 500 lines (complexity risk)
find . -name "*.py" -not -path "./.venv/*" -not -path "./tests/*" -not -path "./mutants/*" -not -path "./__pycache__/*" -exec wc -l {} + | sort -rn | head -10
# Any file over 800 lines warrants investigation. Over 1200 is a finding.

# Duplicated logic — functions with very similar signatures across modules
grep -rn 'def ' --include='*.py' | grep -v test | grep -v __pycache__ | grep -v .venv | sed 's/.*def //' | sed 's/(.*$//' | sort | uniq -d
```
God files aren't automatically bad — `lucyd.py` is the orchestrator and will always be large. But if a non-orchestrator module exceeds 500 lines, ask: can it be split? Are there multiple responsibilities? For duplicated function names across modules: are they implementing the same logic? If so, extract to a shared utility.

### Config Rot
```bash
# .env entries — check each key is actually read by source
grep -oP '^[A-Z_]+' .env 2>/dev/null | while read key; do
  grep -rq "$key" *.py tools/ channels/ providers/ 2>/dev/null || echo "UNUSED .env key: $key"
done

# lucyd.toml keys — check each section/key is consumed by config.py
grep -oP '^\[([a-z_.]+)\]' lucyd.toml 2>/dev/null | tr -d '[]' | while read section; do
  grep -rq "$section" config.py 2>/dev/null || echo "UNCONSUMED config section: [$section]"
done

# Stale feature references — config keys for features that were removed
grep -rn 'config\.' --include='*.py' | grep -v test | grep -v __pycache__ | sed 's/.*config\./config./' | sed 's/[^a-z_].*//' | sort -u > /tmp/config_accessed.txt
# Cross-reference against config.py property definitions
```
Stale .env keys waste operator attention during setup. Unconsumed config sections are dead weight. Config keys accessed in source but not defined in config.py will silently use defaults or crash — both are bad.
