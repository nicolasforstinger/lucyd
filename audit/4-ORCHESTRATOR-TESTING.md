...
```
Any handler that directly mutates state rather than enqueueing a control message is a race condition with the message loop. All state mutations must route through the sequential message queue (AI-002).

### Architectural Invariant Contract Tests
For each AI-NNN invariant, verify a contract test exists that would catch a violation. If the invariant can be verified by grep alone (AI-004), a contract test is optional. If the invariant requires behavioral verification, a contract test is mandatory.

**AI-002 contract test:** Mock the session manager, call each HTTP mutation endpoint, verify the queue received a control message rather than the session being mutated directly.

**AI-003 contract test:** Call a tool that produces output exceeding `output_truncation`, verify the result contains a truncation marker.
...
This extraction has already been completed. The actual function (at `lucyd.py:138`) is:
...
def test_warn_at_threshold():
    assert _should_warn_context(8001, 10000, False, False) is True

def test_no_warn_below_threshold():
    assert _should_warn_context(7999, 10000, False, False) is False

def test_no_warn_already_warned():
    assert _should_warn_context(9999, 10000, False, True) is False

def test_no_warn_needs_compaction():
    assert _should_warn_context(9999, 10000, True, False) is False

def test_exact_boundary():
    assert _should_warn_context(8000, 10000, False, False) is False  # not >
```
