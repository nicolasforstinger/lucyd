...
# Ensure the parent dir (/app in container) is on sys.path so the
# channels package is importable. The bridge is launched via
# ``python -P channels/telegram.py`` which excludes both the script's
# dir and cwd from sys.path; without this, ``channels.bridge_outbound_server``
# cannot be resolved.
...
API_BASE = ""
CHUNK_LIMIT = 4000
CAPTION_LIMIT = 1024  # Telegram media-caption max (vs 4096 text-message limit)
POLL_TIMEOUT = 30
HTTP_TIMEOUT = 45.0
...
async def poll_loop() -> None:
    ...
    _data_dir = os.environ.get("LUCYD_DATA_DIR", "/data")
    download_dir = Path(
        os.environ.get("LUCYD_TELEGRAM_DOWNLOAD_DIR") or f"{_data_dir}/downloads/telegram"
    )
...
async def process_message(message: dict[str, Any], download_dir: Path,
    ...
    # Send typing — cosmetic; failure doesn't block message delivery.
    try:
    ...
    # Skip bundling when the reply exceeds Telegram's caption limit —
    # an oversized caption is rejected with 400, so leave the reply for
    # the chunked text path and send the attachment with no caption.
    caption_text = ""
    if reply and len(outbound_atts) == 1 and len(reply) <= CAPTION_LIMIT:
        caption_text = reply
...
async def _start_outbound_server() -> web.AppRunner:
    ...
    if not ID_TO_NAME:
        log.error("No [telegram.contacts] configured; outbound server refusing to start")
        raise RuntimeError("No telegram contact configured for proactive outbound")
    if len(ID_TO_NAME) > 1:
        log.warning(
            "Multiple telegram contacts configured (%d); outbound will use first key",
            len(ID_TO_NAME),
        )
