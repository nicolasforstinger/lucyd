-- Lucyd initial Postgres schema.
-- Requires the pgvector extension.  Applied at daemon startup by
-- db.ensure_schema() if [database] url_env resolves to a reachable DB.
...
CREATE INDEX idx_sessions_contact ON sessions.sessions (contact);
...
CREATE INDEX idx_messages_session ON sessions.messages (session_id, ordinal);
...
CREATE INDEX idx_events_session ON sessions.events (session_id, created_at);
...
CREATE INDEX idx_facts_entity
    ON knowledge.facts (entity)
    WHERE invalidated_at IS NULL;
CREATE INDEX idx_facts_entity_attr
    ON knowledge.facts (entity, attribute)
    WHERE invalidated_at IS NULL;
...
CREATE INDEX idx_episodes_date ON knowledge.episodes (date);
...
CREATE INDEX idx_commitments_status ON knowledge.commitments (status);
...
CREATE INDEX idx_costs_billing   ON metering.costs (billing_period);
CREATE INDEX idx_costs_timestamp ON metering.costs (timestamp);
...
CREATE INDEX idx_chunks_path ON search.chunks (path);
CREATE INDEX idx_chunks_fts  ON search.chunks USING GIN (search_vector);
