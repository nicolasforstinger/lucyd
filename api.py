...
Endpoints:
    POST /api/v1/chat                       — Operator sync (agentctl)
...
from typing import TYPE_CHECKING, Any  # Any justified: JSON request/response bodies have mixed value types
...
class HTTPApi:
    ...
    def __init__(
        ...
        *,
        ...
        self._bridges_primary = bridges_primary
        self._outbound_http_client = outbound_http_client
        self._session_mgr = session_mgr
        self._pipeline_lock_factory = pipeline_lock_factory
    ...
    async def start(self) -> None:
        ...
        app.router.add_post("/api/v1/outbound/send", self._handle_outbound_send)
    ...
    async def _handle_inbound_telegram(self, request: web.Request) -> web.StreamResponse:
    ...
    async def _handle_inbound_email(self, request: web.Request) -> web.StreamResponse:
    ...
    async def _handle_user_inbound(self, request: web.Request, channel: str) -> web.StreamResponse:
        """Shared bridge handler — talker=user, sender auto-injected.

        Long agentic turns can outlive the bridge's request timeout (e.g.
        Telegram bridge defaults to 300s; deep-thinking turns can exceed
        that). When the bridge has already disconnected by the time the
        daemon writes the reply, fall back to the proactive bridge
        ``/send`` path so the user still gets the message.
        """
        body = await self._parse_body(request)
        if isinstance(body, web.Response):
            return body
        ...
        # Capture original on-disk attachment paths before encoding for HTTP —
        # the bridge fallback re-reads from disk and uses ``data_b64``,
        # while ``_encode_outbound_attachments`` mutates the result in place
        # and produces the HTTP-side ``data`` shape.
        reply_attachment_paths: list[str] = list(result.get("attachments") or [])
        self._encode_outbound_attachments(result)

        return await self._write_inbound_response(
            request, channel, result, reply_attachment_paths,
        )

    async def _write_inbound_response(
        self,
        request: web.Request,
        channel: str,
        result: dict[str, Any],
        reply_attachment_paths: list[str],
    ) -> web.StreamResponse:
        """Write the inbound JSON reply, falling back to the bridge on disconnect.

        Uses StreamResponse so we observe write failures from the handler
        (a plain ``web.Response`` is written by aiohttp after the handler
        returns, hiding the failure). On connection-closed during write,
        if the result carries deliverable user content, push it via the
        bridge's proactive ``/send`` path.
        """
        if self.agent_name:
        ...
        if self.agent_name:
            resp.headers["X-Lucyd-Agent"] = self.agent_name
        resp.content_length = len(body_bytes)

        try:
            await resp.prepare(request)
            await resp.write(body_bytes)
            await resp.write_eof()
            return resp
        except (ConnectionResetError, ConnectionAbortedError, BrokenPipeError,
                asyncio.CancelledError) as e:
            log.warning(
                "HTTP /inbound/%s reply write failed (%s: %s); attempting bridge fallback",
                channel, type(e).__name__, e,
            )
            await self._fallback_to_bridge(channel, result, reply_attachment_paths)
            return resp

    async def _fallback_to_bridge(
        self,
        channel: str,
        result: dict[str, Any],
        reply_attachment_paths: list[str],
    ) -> None:
        """Deliver an inbound reply via the bridge's proactive /send path.

        Used when the bridge that originated the inbound request has
        already disconnected by the time the daemon's reply lands. Only
        fires when there is actual deliverable user content (a non-empty
        ``reply`` text or attachments) and the reply was not silenced or
        an error.
        """
        if result.get("error"):
            return
        ...
        if not text and not reply_attachment_paths:
            return

        if not self._bridges_primary:
            log.warning("/inbound/%s fallback skipped: no primary bridge configured", channel)
            return
        if self._outbound_http_client is None:
            log.warning("/inbound/%s fallback skipped: no outbound http client", channel)
            return

        outbound: list[bridge_client.OutboundAttachment] = []
        for path_str in reply_attachment_paths:
            p = Path(path_str)
            if not p.exists():
                log.warning("/inbound/%s fallback: attachment missing on disk: %s",
                            channel, path_str)
                continue
            ct = self._CONTENT_TYPE_MAP.get(p.suffix.lower(), "application/octet-stream")
            outbound.append({
                "filename": p.name,
                "content_type": ct,
        ...
        try:
            await bridge_client.send_to_user(
                ...
                primary=self._bridges_primary,
                token=self.auth_token,
                http_client=self._outbound_http_client,
            )
            log.info("/inbound/%s reply delivered via bridge fallback (text=%d chars, attachments=%d)",
                     channel, len(text), len(outbound))
        except bridge_client.BridgeDeliveryError as e:
            log.error("/inbound/%s bridge fallback delivery failed: %s", channel, e)
    ...
    async def _handle_outbound_send(self, request: web.Request) -> web.Response:
        """POST /api/v1/outbound/send — at-job callback for remind_user.

        Delivers a literal message to the user via the configured
        primary bridge. Body: ``{"text": str, "attachments": [...]?}``.
        """
        body = await self._parse_body(request)
        if isinstance(body, web.Response):
            return body
        ...
        if not self._bridges_primary:
            return self._json_response(
                {"error": "no primary bridge configured"}, status=503,
            )
        if self._outbound_http_client is None:
            return self._json_response(
                {"error": "outbound http client not configured"}, status=503,
            )
        ...
        try:
            ...
            log.warning("Outbound send failed: %s", e)
            return self._json_response({"error": str(e)}, status=502)

        # Append the outbound to the user's session so a follow-up reply
        # has the message in history. Mirrors send_message tool behavior;
        # without this, remind_user reminders disappear from the agent's
        # context as soon as compaction runs.
        if self._session_mgr and self._pipeline_lock_factory and self.user_name:
            target_key = f"user:{self.user_name}"
            ...
            meta = {
                "from": "outbound_endpoint",
                "fired_at": time.strftime("%Y-%m-%dT%H:%M:%SZ", time.gmtime()),
            }
            try:
                async with self._pipeline_lock_factory(target_key):
                    await self._session_mgr.append_outbound_to_user(
                ...
                log.warning(
                    "Outbound delivered but session append failed: %s", e,
                )

        return self._json_response({"delivered": True}, status=200)
    ...
    async def _handle_plugin_health(self, request: web.Request) -> web.Response:
        ...
        if health is None:
            return self._json_response(
                {"error": f"plugin {name!r} not loaded"}, status=404,
            )
        return self._json_response(health, status=200)
