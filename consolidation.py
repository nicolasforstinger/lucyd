...
async def extract_episode(
    ...
    Returns (episode ID or None, usage object or None).
    """
    ...
    return episode_id, response.usage
...
async def consolidate_session(
    ...
    try:
        facts_added, facts_usage = await extract_facts(
            text, session_id, provider, conn, threshold,
        )

        persona_blocks = context_builder.build_stable()
        episode_id, episode_usage = await extract_episode(
            text, session_id, provider, persona_blocks, conn,
        )
        ...
        with contextlib.suppress(Exception):  # rollback after failed commit; if rollback itself fails, nothing more to do; outer raises
            conn.rollback()
        raise
...
async def extract_from_file(
    ...
    # Record cost (after commit — non-critical)
    if cost_db and cost_rates and usage:
        from agentic import _record_cost
        _record_cost(
