...
async def _store_facts(
    ...
    for fact in data.get("facts", []):
        ...
        entity = await resolve_entity(entity, pool)

        result = await upsert_fact(
            entity, attribute, value, pool,
...
async def extract_facts(
    ...
    count = await _store_facts(data, session_id, pool, confidence_threshold)
    return count, usage
...
async def extract_structured_data(
    ...
    facts_added = await _store_facts(data, session_id, pool, confidence_threshold)
    episode_id = await _store_episode(data, session_id, pool)
...
async def consolidate_session(
    ...
    start_idx, end_idx = await get_unprocessed_range(
        session_id, messages, compaction_count, pool,
    )
    ...
    await update_consolidation_state(
        session_id, compaction_count, len(messages), pool,
    )
