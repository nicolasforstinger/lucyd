...
# ─── PII-safe content redaction ──────────────────────────────────
# When enabled (the default), message content never reaches logs — only a
# length marker. Set once at startup from [logging] pii_safe.

_pii_safe: bool = True


def set_pii_safe(enabled: bool) -> None:
    """Set whether message content may appear in logs. Default: redact."""
    global _pii_safe
    _pii_safe = enabled


def redact_content(text: str | None, max_len: int = 100) -> str:
    """Render message content for a log line, honoring PII-safe mode.

    Default (pii_safe on): emit only a length marker — content never logged.
    Off: emit a control-char-sanitized, truncated preview for debugging.
    """
    if not text:
        return ""
    if _pii_safe:
        return f"[{len(text)} chars, pii_safe]"
    return _log_safe(text[:max_len])
