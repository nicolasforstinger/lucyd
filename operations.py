...
async def update_evolution_state(
    ...
    for fname in ("MEMORY.md", "USER.md"):
        ...
        await pool.execute(
            ...
            "last_evolved_at = now(), content_hash = $2, logs_through = $3",
            fname, content_hash, latest,
        )
...
async def check_new_logs_exist(
    ...
    """
    state = await get_evolution_state(reference_file, pool)
...
async def handle_evolve(
    *,
    ...
    if not force:
        try:
            has_new, since_date = await check_new_logs_exist(config.workspace, pool)
            if not has_new:
