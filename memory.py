...
class MemoryInterface:
    ...
    async def _get_cached_embedding(self, text: str) -> list[float]:
        ...
        except (asyncpg.PostgresError, OSError, TimeoutError, ValueError) as e:
...
async def resolve_entity(name: str, pool: asyncpg.Pool) -> str:
    """Resolve an entity name through the alias table."""
    normalized = _normalize_entity(name)
    row = await pool.fetchrow(
        "SELECT canonical FROM knowledge.entity_aliases WHERE alias = $1",
        normalized,
    )
...
async def lookup_facts(
    ...
    placeholders = ", ".join(f"${i + 1}" for i in range(len(entity_list)))
    limit_idx = len(entity_list) + 1
...
async def recall(
    ...
    if entities:
        facts = await lookup_facts(entities, pool, max_results=max_facts)
        if facts:
