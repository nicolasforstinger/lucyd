...
class MeteringDB:
    ...
    One database per agent (``lucyd_{client}_{agent}``) — client/agent
    identity is implicit in the connection, not carried as columns.
    ...
    async def query(self, sql: str, *args: object) -> list[asyncpg.Record]:
