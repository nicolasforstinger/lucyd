...
class Session:
    ...
    async def repair_trailing_user_orphan(self) -> bool:
        """Drop a trailing user message left orphaned by a mid-loop daemon shutdown.

        When the daemon dies (SIGTERM/SIGKILL/fatal exception) between
        ``add_user_message`` and ``_persist_response``, the persisted message
        list ends with a ``role=user`` row — the agentic loop's response was
        only ever in-memory.  On the next user turn ``add_user_message``
        would raise ``ConsecutiveRoleError`` and the session would be wedged
        until an outbound agent message restored alternation.

        Call once after ``load()`` to detect and repair such orphans.
        Returns ``True`` if a repair was performed.
        """
        if not self.messages or self.messages[-1]["role"] != "user":
            return False
        orphan_ordinal = len(self.messages) - 1
        self.messages.pop()
        await self._pool.execute(
            "DELETE FROM sessions.messages "
            "WHERE session_id = $1 AND ordinal = $2",
            self.id, orphan_ordinal,
        )
        self._persisted_count = len(self.messages)
        log.warning(
            "Session %s: repaired trailing orphan user message at ordinal %d "
            "(daemon likely shut down mid-loop)",
            self.id, orphan_ordinal,
        )
        if metrics.ENABLED:
            metrics.ERRORS_TOTAL.labels(error_type="orphan_user_repaired").inc()
        return True
                ...
                for i in range(self._persisted_count, len(self.messages)):
                    ...
                    await conn.execute(
                        """INSERT INTO sessions.messages
                           (session_id, role, content, ordinal)
    ...
    async def append_event(self, event: dict[str, Any]) -> None:
        ...
        await self._pool.execute(
            """INSERT INTO sessions.events
               (session_id, event_type, payload, trace_id)
               VALUES ($1, $2, $3::jsonb, $4)""",
            self.id,
...
class SessionManager:
    ...
    async def session_count(self) -> int:
        ...
        val = await self._pool.fetchval(
            "SELECT COUNT(*) FROM sessions.sessions WHERE closed_at IS NULL",
        )
        return val or 0
    ...
    async def get_or_create(self, contact: str, model: str = "") -> Session:
        ...
        if row:
            ...
            if await session.load():
                # Repair any orphan user message left by a mid-loop daemon
                # shutdown — see Session.repair_trailing_user_orphan.
                await session.repair_trailing_user_orphan()
    ...
    async def append_outbound_to_user(
        self,
        target_key: str,
        text: str,
        ...
        """Append a proactive AssistantMessage to a target session.

        Used by ``send_message`` (called from agent:self turns) to write
        the outbound into the user's session history so the user's next
        reply has full context.

        - Loads or creates the target session.
        - If the session has no prior messages, prepends a synthetic
          anchor UserMessage so provider role-alternation holds for the
          next turn.
        - Appends an AssistantMessage with the outbound text and any
          attachment references (paths + metadata, not raw bytes).
        - Tags the message with ``source_metadata`` (e.g. ``{"from":
          "agent_self_send_message", "fired_at": iso}``) so operators can
          distinguish proactive outbounds from in-turn replies.

        Concurrency: the caller is responsible for acquiring
        ``pipeline.get_session_lock(target_key)`` before invoking this
        method. ``SessionManager`` itself does not own the pipeline
        reference.
        """
        session = await self.get_or_create(target_key)

        if not session.messages:
            anchor: UserMessage = {
                "role": "user",
                "content": (
                    "[external trigger: agent acted on a scheduled task and "
                    "sent you the following message]"
                ),
            }
            session.messages.append(anchor)

        outbound: AssistantMessage = {"role": "agent", "text": text}
        if attachment_refs:
            outbound["attachments"] = attachment_refs
        if source_metadata:
            outbound["source_metadata"] = source_metadata
