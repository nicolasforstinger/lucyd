...
class TestCompactFlag:
    """Tests for --compact / forced diary + compaction."""

    def test_compact_sends_correct_fifo_message(self, tmp_path):
        """--compact sends compact FIFO message."""
        fifo_path = tmp_path / "control.pipe"
        os.mkfifo(str(fifo_path))
    ...
    def test_compact_no_fifo_exits(self, tmp_path, monkeypatch):
        """--compact with no FIFO exits with error."""
        monkeypatch.setattr("sys.argv", [
            "lucyd-send", "--compact",
            "--state-dir", str(tmp_path),
        ])
