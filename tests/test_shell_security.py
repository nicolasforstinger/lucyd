...
class TestExecTimeoutProcessEnforcement:
    ...
    async def test_process_group_killed_not_just_parent(self, monkeypatch):
        ...
        shell_pid = pids[0]
        # Shell process should be dead.  Allow a brief window for the kernel
        # to reap zombies — SIGKILL is delivered immediately but the process
        # table entry lingers until the parent waits.
        ...
        # The child 'sleep 300' process should also be dead.
        # os.killpg sends SIGKILL to the entire group via start_new_session=True.
        try:
            os.killpg(shell_pid, 0)
            pytest.fail(f"Process group {shell_pid} still has live processes")
        except OSError:
            pass  # Expected — entire group is dead
