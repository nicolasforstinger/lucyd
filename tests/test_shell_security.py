...
class TestSafeEnvSuffix:
    ...
    def test_filters_key_suffix(self, monkeypatch):
        """ENCRYPTION_KEY ends with _KEY."""
        monkeypatch.setenv("ENCRYPTION_KEY", "aes256secret")
        env = _safe_env()
        assert "ENCRYPTION_KEY" not in env

    def test_filters_key_id_suffix(self, monkeypatch):
        """AWS_ACCESS_KEY_ID ends with _KEY_ID."""
...
class TestSecretPatterns:
    ...
    def test_suffixes(self):
        ...
        assert "_KEY_ID" in _SECRET_SUFFIXES
...
class TestExecTimeoutProcessEnforcement:
    """End-to-end: real subprocess, real timeout, verify process is dead.

    Closes the audit gap: _safe_env is mutation-verified, but the actual
    timeout-kills-the-process path was only tested via mocks (TestExecKillChain)
    or only checked the return message (TestExecTimeout). These tests use real
    subprocesses with short timeouts and verify the OS-level process is gone.
    """

    @pytest.mark.asyncio
    async def test_real_process_killed_after_timeout(self, monkeypatch):
        """A real long-running process is killed when timeout fires.

        Spawns 'sleep 300' with 1s timeout, then checks that the PID
        is no longer alive. This exercises the full path:
        asyncio.wait_for → TimeoutError → os.killpg → proc.wait.
        """
        import tools.shell as shell_mod
        monkeypatch.setattr(shell_mod, "_MAX_TIMEOUT", 1)
        monkeypatch.setattr(shell_mod, "_DEFAULT_TIMEOUT", 1)

        # Capture the PID from the real subprocess
        pids = []
        original_create = __import__("asyncio").create_subprocess_shell
        ...
        with patch("asyncio.create_subprocess_shell", side_effect=spy_create):
            result = await tool_exec("sleep 300")

        assert "timed out" in result.lower()
        assert len(pids) == 1

        pid = pids[0]
        # The process should be dead — os.kill(pid, 0) raises OSError if gone
        try:
            os.kill(pid, 0)
            pytest.fail(f"Process {pid} still alive after timeout kill")
        except OSError:
            pass  # Expected — process is dead

    @pytest.mark.asyncio
    async def test_process_group_killed_not_just_parent(self, monkeypatch):
        """Verify that child processes in the group are also killed.

        Spawns a shell that forks a background child ('sleep 300 &'),
        then verifies both the parent shell and the child sleep are dead.
        This tests that start_new_session=True + os.killpg kills the group.
        """
        import tools.shell as shell_mod
        monkeypatch.setattr(shell_mod, "_MAX_TIMEOUT", 1)
        monkeypatch.setattr(shell_mod, "_DEFAULT_TIMEOUT", 1)
        ...
        # Shell forks a background child, writes its PID, then waits
        cmd = "sleep 300 & echo $!; wait"
        with patch("asyncio.create_subprocess_shell", side_effect=spy_create):
            result = await tool_exec(cmd)
        ...
        # Poll until the shell process is gone.
        ...
        if not await _wait_dead(shell_pid):
            pytest.fail(f"Shell process {shell_pid} still alive after group kill")

        # Poll until no LIVING members of the group remain.  os.killpg(pid, 0)
        # would also succeed for zombies (state=Z), and in nested container
        # CI environments init can take seconds to reap them.  Zombies are
        # already-dead processes — the test's intent is "no live members",
        # so walk /proc and exclude state=Z.
        def _live_group_members() -> list[str]:
            alive: list[str] = []
            for p in os.listdir("/proc"):
                ...
                try:
                    ...
                    pgid = int(fields[4])
                    state = fields[2]
                    if pgid == shell_pid and state != "Z":
                        with open(f"/proc/{p}/comm") as f:
                            comm = f.read().strip()
                        alive.append(f"pid={p} comm={comm} state={state}")
        ...
        while asyncio.get_event_loop().time() < deadline:
            members = _live_group_members()
            if not members:
                break
            await asyncio.sleep(0.05)
        else:
            pytest.fail(
                f"Process group {shell_pid} still has live (non-zombie) "
                f"processes after 5s: {_live_group_members()}"
            )

    @pytest.mark.asyncio
    async def test_timeout_returns_within_reasonable_time(self, monkeypatch):
        """Timeout enforcement completes promptly, not waiting for the full command.

        A 'sleep 300' with 1s timeout should return in ~1-2s, not 300s.
        This ensures asyncio.wait_for actually fires (not just mocked).
        """
        import time
        ...
        assert "timed out" in result.lower()
        # Should complete in ~1-3s (1s timeout + kill overhead), definitely not 300s
        assert elapsed < 5, f"Timeout took {elapsed:.1f}s — expected ~1-2s"

    @pytest.mark.asyncio
    async def test_start_new_session_creates_process_group(self, monkeypatch):
        """Verify start_new_session=True creates a new process group.

        The shell spawned by create_subprocess_shell becomes the session
        leader (its PID == its PGID). The child inherits that PGID.
        Crucially, the child's PGID must differ from the TEST process's
        PGID — proving the subprocess is in an isolated group that killpg
        can target without affecting the test runner.
        """
        import tools.shell as shell_mod
        monkeypatch.setattr(shell_mod, "_MAX_TIMEOUT", 5)
        monkeypatch.setattr(shell_mod, "_DEFAULT_TIMEOUT", 5)

        test_pgid = os.getpgid(0)

        # Ask the subprocess to report its own PGID and parent's PID
        result = await tool_exec(
            "python3 -c \"import os; print(os.getpgid(0)); print(os.getppid())\""
        )
        lines = result.strip().split("\n")
        child_pgid = int(lines[0])
        child_ppid = int(lines[1])  # Parent is the shell — the session leader

        # Child's PGID should equal its parent shell's PID (session leader)
        assert child_pgid == child_ppid, (
            f"Child PGID {child_pgid} != parent shell PID {child_ppid}"
        )
        # And that group must be different from the test process's group
        assert child_pgid != test_pgid, (
            f"Child PGID {child_pgid} == test PGID {test_pgid} — "
            "start_new_session not effective, killpg would hit test runner"
        )
...
class TestExecKillChain:
    ...
    async def test_timeout_calls_killpg(self):
        ...
        proc.communicate = MagicMock(return_value="communicate")
