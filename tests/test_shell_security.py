...
class TestExecTimeoutProcessEnforcement:
    ...
    async def test_process_group_killed_not_just_parent(self, monkeypatch):
        ...
        # It might take a moment for the kernel to reap, but killpg sends
        # SIGKILL to the entire group so it should be immediate.
        # We can't reliably get the child PID from the timed-out output,
        # so we verify no process in the session group survives.
        # os.killpg would have already sent SIGKILL to pgid=shell_pid.
        try:
