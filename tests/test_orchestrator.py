...
class TestBasicMessageFlow:
    ...
    async def test_reply_delivered_to_channel(self, tmp_path):
        ...
        with patch("lucyd.run_agentic_loop", side_effect=fake_loop), patch("tools.status.set_current_session"):
...
class TestDocumentExtractionIntegration:
    ...
    async def test_text_document_injected(self, tmp_path):
        ...
        with patch("lucyd.run_agentic_loop", return_value=response), patch("tools.status.set_current_session"):
            await daemon._process_message(
...
class TestImageFitting:
    ...
    def test_png_too_large_raises(self):
        ...
        from lucyd import _fit_image, _ImageTooLarge
...
class TestEnrichImageCaption:
    """Unit tests for _enrich_image_caption."""

    def test_basic_replacement(self):
        """Tag replaced with truncated assistant description."""
        messages = [
            {"role": "user", "content": "[Image from user] look at this"},
            {"role": "assistant", "text": "a sunset over mountains with orange sky"},
        ]
    ...
    def test_multiline_collapsed(self):
        """Newlines in description collapsed to spaces."""
        messages = [
    ...
    def test_msg_count_before_offset(self):
        """Only looks at messages from msg_count_before onward."""
        messages = [
            {"role": "assistant", "text": "old response"},  # before
...
class TestForcedCompact:
    ...
    async def test_force_compact_triggers_compaction_under_threshold(self, tmp_path):
        ...
        with patch("lucyd.run_agentic_loop", side_effect=fake_loop), \
             patch("tools.status.set_current_session"):
            await daemon._process_message(
                text="Write diary", sender="Nicolas", source="system",
                force_compact=True,
            )
    ...
    async def test_force_compact_does_not_auto_close(self, tmp_path):
        ...
        with patch("lucyd.run_agentic_loop", side_effect=fake_loop), \
             patch("tools.status.set_current_session"):
            await daemon._process_message(
                text="compact diary", sender="Nicolas", source="system",
                force_compact=True,
            )
