...
class TestConsecutiveUserMessageRejection:
    ...
    async def test_consecutive_user_message_rejected(self, tmp_path):
        ...
        # Self-heals: the request is resolved with a clear error, not left hanging.
        assert fut.done()
        assert "consecutive user messages" in fut.result().get("error", "")
...
class TestImageFitting:
    ...
    def test_b64_size_check_catches_inflated_png(self):
        """PNG that's <= max_bytes raw but > max_bytes base64 must raise.

        Regression for 2026-05-06 17:10 UTC silent-drop incident: Anthropic's
        image size cap is enforced on the base64-encoded payload, not the
        raw bytes. A 4.9 MiB raw PNG produces ~6.5 MiB base64 and is
        rejected by the API. fit_image must compare in encoded space so
        such an image surfaces as ImageTooLarge instead of silently
        flowing through to the provider.
        """
        import random
        from io import BytesIO
        ...
        # Build a noisy PNG that won't compress well — 1200x1200 random RGB
        # lands at ~4.3 MiB raw / ~5.7 MiB base64, the bug's exact zone.
        ...
        # Confirm the bug shape: raw fits, base64 doesn't.
        raw_len = len(data)
        b64_len = ((raw_len + 2) // 3) * 4
        assert raw_len <= 5 * 1024 * 1024, f"test fixture must be <=5MiB raw; got {raw_len}"
        assert b64_len > 5 * 1024 * 1024, f"test fixture must be >5MiB base64; got {b64_len}"

        # Old behavior: returned data silently (raw <= max_bytes).
        # New behavior: raises because base64 exceeds the cap.
        with pytest.raises(_ImageTooLarge):
            _fit_image(data, "image/png", 5 * 1024 * 1024, 1568)
...
class TestAutoCloseSystemSessions:
    ...
    async def test_system_source_triggers_close(self, tmp_path):
        ...
        with patch("pipeline.run_agentic_loop", side_effect=fake_loop):
            await daemon._process_message(
                text="maintain", sender="maintenance", talker="system",
            )

        daemon.session_mgr.close_session.assert_called_once_with("system:maintenance")
    ...
    async def test_system_error_still_closes(self, tmp_path):
        ...
        daemon.session_mgr.close_session.assert_awaited_once_with("system:maintenance")
...
class TestForcedCompact:
    ...
    async def test_handle_compact_skips_when_no_user_session(self, tmp_path):
        ...
        daemon.session_mgr._index = {"system:maintenance": evo_session, "system:automation": evo_session}
        daemon.session_mgr.list_contacts = AsyncMock(return_value=["system:maintenance", "system:automation"])
