...
class TestSingleProviderArchitecture:
    ...
    def test_get_provider_returns_primary_by_default(self):
        """get_provider("compaction") returns the primary provider when no routing override.
        ...
        """
        source = (_LUCYD_DIR / "lucyd.py").read_text(encoding="utf-8")
        # get_provider must have a fallback to the primary provider. Accept either
        # `return self.provider` (old form) or `return self._providers["primary"]`
        # (post-type-tightening form).
        assert (
            "return self.provider" in source
            or 'return self._providers["primary"]' in source
        ), "get_provider() missing fallback to the primary provider (AI-005)"
...
class TestPersonaBudgetValidation:
    """C5: persona size validated against the model context window at startup.

    Over 80% of the window → error; 50–80% → warning; below → info only.
    """

    def _validate(self, tmp_path, target_pct, caplog):
        import logging
        ...
        ws = tmp_path / "ws"
        ws.mkdir()
        persona = "context budget word " * 500
        (ws / "SOUL.md").write_text(persona)
        # Size the window so the persona lands at the target fraction —
        # robust to tokenizer variance.
        max_ctx = max(1, int(_estimate_tokens(persona) / target_pct))
        cb = ContextBuilder(workspace=ws, stable_files=["SOUL.md"], semi_stable_files=[])
        stub = SimpleNamespace(
            provider=SimpleNamespace(
                capabilities=SimpleNamespace(max_context_tokens=max_ctx),
            ),
            context_builder=cb,
        )
        with caplog.at_level(logging.WARNING):
            LucydDaemon._validate_persona_budget(stub)
        return caplog.records

    def test_persona_over_80pct_logs_error(self, tmp_path, caplog):
        import logging
        records = self._validate(tmp_path, 0.9, caplog)
        assert any(r.levelno == logging.ERROR for r in records)
        assert "starved" in " ".join(r.getMessage() for r in records)

    def test_persona_50_to_80pct_logs_warning(self, tmp_path, caplog):
        import logging
        records = self._validate(tmp_path, 0.6, caplog)
        assert any(r.levelno == logging.WARNING for r in records)
        assert not any(r.levelno == logging.ERROR for r in records)
        assert "limited room" in " ".join(r.getMessage() for r in records)

    def test_persona_under_50pct_no_warning(self, tmp_path, caplog):
        import logging
        records = self._validate(tmp_path, 0.2, caplog)
