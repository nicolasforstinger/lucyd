...
class TestMemoryWrite:
    def test_creates_new_fact(self, mem_conn):
        result = handle_memory_write("nicolas", "favorite_color", "blue")
    ...
    def test_deduplicates_same_value(self, seeded_conn):
        result = handle_memory_write("nicolas", "lives_in", "Austria")
    ...
    def test_updates_changed_value(self, seeded_conn):
        result = handle_memory_write("nicolas", "lives_in", "Germany")
    ...
    def test_resolves_alias(self, seeded_conn):
        result = handle_memory_write("nic", "age", "30")
        assert "nicolas.age" in result  # resolved from "nic" alias

    def test_normalizes_entity(self, mem_conn):
        result = handle_memory_write("Uncle Charles", "relation", "uncle")
        assert "uncle_charles" in result

    def test_not_configured_returns_error(self):
        configure(None)
        result = handle_memory_write("test", "attr", "val")
...
class TestMemoryForget:
    def test_sets_invalidated_at(self, seeded_conn):
        result = handle_memory_forget("nicolas", "lives_in")
    ...
    def test_no_match_returns_not_found(self, seeded_conn):
        result = handle_memory_forget("nicolas", "nonexistent_attr")
        assert "No current fact found" in result

    def test_resolves_alias(self, seeded_conn):
        result = handle_memory_forget("nic", "lives_in")
        assert "Forgotten:" in result
        assert "nicolas" in result

    def test_not_configured_returns_error(self):
        configure(None)
        result = handle_memory_forget("test", "attr")
        assert "Error" in result
...
class TestCommitmentUpdate:
    def test_changes_status(self, seeded_conn):
        ...
        result = handle_commitment_update(cid, "done")
    ...
    def test_no_match_returns_not_found(self, seeded_conn):
        result = handle_commitment_update(9999, "done")
        assert "No open commitment" in result

    def test_only_affects_open(self, seeded_conn):
        ...
        result = handle_commitment_update(cid, "cancelled")
        assert "No open commitment" in result
    ...
    def test_not_configured_returns_error(self):
        configure(None)
        result = handle_commitment_update(1, "done")
        assert "Error" in result
