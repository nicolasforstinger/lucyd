...
class TestMaintainConfig:
    """[maintain] self-maintenance heartbeat config defaults and overrides."""
...
class TestResolvedIdentity:
    """resolved_agent_id falls back to agent_name."""
...
class TestBridgesConfig:
    """[bridges] section exposes the active outbound primary."""

    def test_bridges_primary_default_empty(self, minimal_toml_data):
        """If [bridges] is absent, bridges_primary defaults to ''."""
        minimal_toml_data.pop("bridges", None)
        cfg = Config(minimal_toml_data)
        assert cfg.bridges_primary == ""

    def test_bridges_primary_explicit(self, minimal_toml_data):
        minimal_toml_data["bridges"] = {"primary": "telegram"}
        cfg = Config(minimal_toml_data)
        assert cfg.bridges_primary == "telegram"
