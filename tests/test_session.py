...
class TestOrphanUserRepair:
    """Resume-time repair of mid-loop-shutdown orphan user messages.

    When the daemon dies between ``add_user_message`` and
    ``_persist_response`` (SIGTERM / fatal exception / SIGKILL), the DB
    is left with a trailing ``role=user`` row.  On the next user turn
    ``add_user_message`` would raise ``ConsecutiveRoleError`` and wedge
    the session.  ``Session.repair_trailing_user_orphan`` and the
    ``SessionManager.get_or_create`` resume path repair this.
    """

    @pytest.mark.asyncio
    async def test_repair_pops_trailing_user_message(self, pool):
        session = await _create_session(pool, "orphan-1", contact="user:test")
        await session.add_user_message("hello", sender="test")
        # Simulate a normal turn that completed.
        session.messages.append({
            "role": "agent", "text": "hi",
            "usage": {"input_tokens": 5, "output_tokens": 3},
        })
        await session.save_state()
        # Simulate mid-loop crash on the next user message: user persisted,
        # agentic loop never finalized.
        await session.add_user_message("are you there?", sender="test")
        assert session.messages[-1]["role"] == "user"

        # Reload as if the daemon restarted.
        reloaded = Session("orphan-1", pool)
        assert await reloaded.load() is True
        assert len(reloaded.messages) == 3
        assert reloaded.messages[-1]["role"] == "user"

        repaired = await reloaded.repair_trailing_user_orphan()
        assert repaired is True
        assert len(reloaded.messages) == 2
        assert reloaded.messages[-1]["role"] == "agent"

        # DB row is gone — a third reload sees the repaired state.
        again = Session("orphan-1", pool)
        await again.load()
        assert len(again.messages) == 2
        assert again.messages[-1]["role"] == "agent"

    @pytest.mark.asyncio
    async def test_repair_noop_when_last_message_is_agent(self, pool):
        session = await _create_session(pool, "orphan-2", contact="user:test")
        await session.add_user_message("hello", sender="test")
        session.messages.append({
            "role": "agent", "text": "hi",
            "usage": {"input_tokens": 5, "output_tokens": 3},
        ...
        reloaded = Session("orphan-2", pool)
        await reloaded.load()
        repaired = await reloaded.repair_trailing_user_orphan()
    ...
    @pytest.mark.asyncio
    async def test_repair_noop_on_empty_session(self, pool):
        session = await _create_session(pool, "orphan-3", contact="user:test")
        repaired = await session.repair_trailing_user_orphan()
        assert repaired is False
        assert len(session.messages) == 0

    @pytest.mark.asyncio
    async def test_repair_allows_next_user_message_to_succeed(self, pool):
        """End-to-end: orphan user → reload → repair → next add_user_message OK."""
        session = await _create_session(pool, "orphan-4", contact="user:test")
        await session.add_user_message("first", sender="test")
        # Crash before any agent reply.

        reloaded = Session("orphan-4", pool)
        await reloaded.load()
        await reloaded.repair_trailing_user_orphan()

        # Next user message must not raise ConsecutiveRoleError.
        await reloaded.add_user_message("retry", sender="test")
        assert len(reloaded.messages) == 1
        assert reloaded.messages[-1]["content"] == "retry"

    @pytest.mark.asyncio
    async def test_session_manager_repairs_on_resume(self, pool):
        """SessionManager.get_or_create runs the repair on a freshly loaded session."""
        # Set up a session with a persisted orphan user message.
        seed = await _create_session(pool, "orphan-5", contact="user:resume")
        await seed.add_user_message("hello", sender="test")

        # New SessionManager instance — no in-memory cache; resume forces DB load.
        mgr = SessionManager(pool)
        session = await mgr.get_or_create("user:resume")
        assert session.id == "orphan-5"
        assert len(session.messages) == 0

        # The DB-side row is gone; a follow-up user message goes through.
        await session.add_user_message("retry", sender="test")
