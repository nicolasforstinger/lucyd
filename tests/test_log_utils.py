...
class TestRedactContent:
    """PII-safe content redaction (Cornerstone 10)."""

    def test_pii_safe_omits_content(self, monkeypatch):
        import log_utils
        monkeypatch.setattr(log_utils, "_pii_safe", True)
        out = log_utils.redact_content("my SSN is 123-45-6789")
        assert "123-45-6789" not in out
        assert "chars" in out  # length marker only

    def test_disabled_shows_truncated_preview(self, monkeypatch):
        import log_utils
        monkeypatch.setattr(log_utils, "_pii_safe", False)
        assert "hello world" in log_utils.redact_content("hello world")

    def test_disabled_sanitizes_control_chars(self, monkeypatch):
        import log_utils
        monkeypatch.setattr(log_utils, "_pii_safe", False)
        out = log_utils.redact_content("line1\nline2")
        assert "\n" not in out  # forged-log control chars stripped even in preview

    def test_empty_returns_empty(self):
        import log_utils
        assert log_utils.redact_content("") == ""
        assert log_utils.redact_content(None) == ""
