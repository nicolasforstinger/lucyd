...
class TestToolTts:
    ...
    @pytest.mark.asyncio
    async def test_sdk_not_installed_raises(self) -> None:
        elevenlabs_plugin._client = None
        with patch.object(elevenlabs_plugin, "AsyncElevenLabs", None):
            with pytest.raises(PluginNotConfigured, match="not configured"):
    ...
    @pytest.mark.asyncio
    async def test_missing_voice_id_raises(self) -> None:
        elevenlabs_plugin._client = MagicMock()
        elevenlabs_plugin._default_voice_id = ""
        with pytest.raises(PluginInvalidInput, match="voice_id"):
            await elevenlabs_plugin.tool_tts(text="Hello")

    @pytest.mark.asyncio
    async def test_api_error_raises_upstream(self, tmp_path: Path) -> None:
        ...
        with patch("plugins.asyncio.sleep", new=AsyncMock()), \
             pytest.raises(PluginUpstream, match="RuntimeError"):
            await elevenlabs_plugin.tool_tts(text="Hello")
