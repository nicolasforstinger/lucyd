...
class TestProcessMessage:
    ...
    @pytest.mark.asyncio
    async def test_short_reply_bundled_into_caption(self, tmp_path: Path) -> None:
        """reply <= CAPTION_LIMIT + 1 attachment → bundled as caption, no text send."""
        tg._bot_id = 0
        tg.ALLOWED_SENDERS = set()
        tg.DAEMON_URL = "http://daemon:8100"

        reply = "x" * 100
        atts = [_b64_attachment("v.ogg", b"oggdata", "audio/ogg")]
        mock_client = AsyncMock(spec=httpx.AsyncClient)
        mock_client.post.return_value = _daemon_response(reply, attachments=atts)
        ...
        async def capture_send(_chat_id: int, path: str, **kwargs: object) -> None:
            captions.append(str(kwargs.get("caption", "")))

        with (
            patch.object(tg, "_get_client", return_value=mock_client),
            patch.object(tg, "tg_api", new_callable=AsyncMock),
            patch.object(tg, "send_text", new_callable=AsyncMock) as mock_st,
            patch.object(tg, "send_attachment", side_effect=capture_send),
            patch.object(tg, "extract_attachments", new_callable=AsyncMock, return_value=[]),
        ):
            await tg.process_message(_msg(), tmp_path)
    ...
    @pytest.mark.asyncio
    async def test_oversized_reply_not_bundled_into_caption(self, tmp_path: Path) -> None:
        """reply > CAPTION_LIMIT + 1 attachment → text sent separately, empty caption.

        Regression for the 2026-05-17 sendVoice 'caption is too long' incident:
        an oversized reply must NOT be consumed into the attachment caption.
        """
        tg._bot_id = 0
        tg.ALLOWED_SENDERS = set()
        tg.DAEMON_URL = "http://daemon:8100"

        reply = "x" * (tg.CAPTION_LIMIT + 500)
        atts = [_b64_attachment("v.ogg", b"oggdata", "audio/ogg")]
        mock_client = AsyncMock(spec=httpx.AsyncClient)
        mock_client.post.return_value = _daemon_response(reply, attachments=atts)
        ...
        # Full text delivered via the chunked text path (send_text chunks it)...
        mock_st.assert_called_once()
        assert mock_st.call_args.args[1] == reply
        # ...and the attachment carried no caption (so no oversized-caption 400).
        assert captions == [""]
        for cap in captions:
