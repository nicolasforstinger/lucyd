...
class TestSmokeLocalProvider:
    def test_create_provider_supports_smoke_local(self):
        from providers.smoke_local import SmokeLocalProvider

        provider = create_provider({"provider": "smoke-local", "model": "smoke-model"})
        assert isinstance(provider, SmokeLocalProvider)
        assert provider.capabilities.supports_tools is False

    @pytest.mark.asyncio
    async def test_smoke_local_returns_deterministic_reply(self):
        provider = create_provider({
            "provider": "smoke-local",
            "model": "smoke-model",
            "reply_text": "SMOKE_TEST_OK",
        })
    ...
    @pytest.mark.asyncio
    async def test_smoke_local_uses_shared_token_estimator(self):
        ...
        assert result.usage.input_tokens == (
            _estimate_tokens("system prompt") + _estimate_tokens("hi")
        )
        assert result.usage.output_tokens == _estimate_tokens("SMOKE_TEST_OK")
...
class TestModelCapabilities:
    """Verify ModelCapabilities dataclass and provider integration."""
    ...
    def test_anthropic_provider_has_capabilities(self):
        ...
        assert p.capabilities.max_context_tokens == 200000
    ...
    def test_openai_provider_has_capabilities(self):
        ...
        assert isinstance(p.capabilities, ModelCapabilities)
        assert p.capabilities.supports_tools is False
        assert p.capabilities.max_context_tokens == 8192

    def test_thinking_inferred_from_thinking_mode(self):
        ...
        assert p.capabilities.supports_thinking is True
    ...
    def test_supports_tools_defaults_true(self):
...
class TestStreamDelta:
    ...
    def test_tool_delta(self):
        d = StreamDelta(tool_call_index=0, tool_call_id="tc1", tool_name="read")
        assert d.tool_call_index == 0
        assert d.tool_name == "read"

    def test_final_delta(self):
        d = StreamDelta(stop_reason="end_turn", usage=Usage(input_tokens=100, output_tokens=50))
        assert d.stop_reason == "end_turn"
        assert d.usage.input_tokens == 100


class TestStreamFallback:
    """Verify stream_fallback yields a single delta from complete()."""

    async def test_yields_single_delta(self):
        """stream_fallback calls complete() and yields one StreamDelta."""
        ...
        deltas = []
        async for d in stream_fallback(mock_provider, "sys", [], []):
            deltas.append(d)

        assert len(deltas) == 1
        assert deltas[0].text == "Hello world"
        assert deltas[0].thinking == "thought"
        assert deltas[0].stop_reason == "end_turn"
        assert deltas[0].usage.output_tokens == 5
