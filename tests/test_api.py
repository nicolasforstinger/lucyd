...
def _make_app(api_instance: HTTPApi) -> web.Application:
    ...
    app.router.add_post("/api/v1/outbound/send", api_instance._handle_outbound_send)
...
class TestQueueRoutingInvariant:
    ...
    _NON_QUEUEING_HANDLERS = frozenset({
        ...
        "_handle_outbound_send",     # outbound to bridge — synchronous, no queue
    })
    ...
    def _handler_uses_queue(method_name: str) -> tuple[bool, str]:
        ...
        for node in ast.walk(tree):
            ...
            # self._handle_xxx_cb() etc — callback delegation
