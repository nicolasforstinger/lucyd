...
_DEFAULT_SUBAGENT_DENY = frozenset({"sessions_spawn"})
...
def setup_agents(mock_registry, mock_provider, mock_config):
    ...
    original = (mod._config, mod._provider, mod._tool_registry,
                mod._subagent_deny, mod._default_max_turns, mod._default_timeout)
    ...
    yield
    (mod._config, mod._provider, mod._tool_registry,
     mod._subagent_deny, mod._default_max_turns, mod._default_timeout) = original
...
class TestDenyListRealFunction:
    ...
    async def test_explicit_tools_blocks_exec(self):
        """Explicit tools=['exec', 'read']: exec is NOT in _DEFAULT_SUBAGENT_DENY."""
        mock_loop = AsyncMock(return_value=_make_mock_response())
        with patch("agentic.run_agentic_loop", mock_loop):
        ...
        tool_names = {t["name"] for t in tools_passed}
        assert "read" in tool_names
        # exec is NOT in _DEFAULT_SUBAGENT_DENY by default
        # _DEFAULT_SUBAGENT_DENY = {"sessions_spawn"}
    ...
    async def test_non_denied_tools_pass_through(self):
        ...
        mock_loop = AsyncMock(return_value=_make_mock_response())
        with patch("agentic.run_agentic_loop", mock_loop):
            await tool_sessions_spawn(prompt="test", tools=["read", "write"])
...
class TestSpawnReturnValue:
    ...
    @pytest.mark.asyncio
    async def test_error_for_no_provider(self):
        """No provider configured returns error."""
        import tools.agents as mod
...
class TestConfigDefaults:
    ...
    async def test_default_max_turns_from_config(self):
        ...
        assert call_kwargs["config"].max_turns == 50
    ...
    async def test_explicit_max_turns_overrides_default(self):
        ...
        call_kwargs = mock_loop.call_args.kwargs
        assert call_kwargs["config"].max_turns == 25
    ...
    async def test_default_timeout_from_config(self):
        ...
        with patch("agentic.run_agentic_loop", mock_loop):
            await tool_sessions_spawn(prompt="test")
        call_kwargs = mock_loop.call_args.kwargs
        assert call_kwargs["config"].timeout == 600.0
    ...
    async def test_explicit_timeout_overrides_default(self):
        ...
        call_kwargs = mock_loop.call_args.kwargs
        assert call_kwargs["config"].timeout == 30.0
