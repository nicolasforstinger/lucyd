...
class TestPathValidation:
    ...
    def test_error_lists_allowed_prefixes(self, tmp_path):
        ...
        try:
            ...
            # Verify allowed prefixes are NOT leaked to the LLM
            assert "/allowed/a" not in result
            assert "/allowed/b" not in result
        finally:
            filesystem._PATH_ALLOW = original
...
# ─── Operator-owned write-guard ──────────────────────────────────

class TestLockedFileGuard:
    """write/edit unconditionally refuse the operator-owned protocol files."""

    LOCKED = ["SOUL.md", "AGENTS.md", "TOOLS.md", "MAINTAIN.md"]

    @pytest.mark.parametrize("name", LOCKED)
    def test_write_refuses_locked_file(self, fs_workspace, name):
        # The path is inside the allowlist — the guard still refuses it.
        path = str(fs_workspace / name)
        result = tool_write(path, "tampered")
        assert result.startswith("Error:")
        assert "operator-owned" in result
        assert not (fs_workspace / name).exists()

    @pytest.mark.parametrize("name", LOCKED)
    def test_edit_refuses_locked_file(self, fs_workspace, name):
        target = fs_workspace / name
        target.write_text("original line\n")
        result = tool_edit(str(target), "original", "tampered")
        assert result.startswith("Error:")
        assert "operator-owned" in result
        assert target.read_text() == "original line\n"

    def test_guard_fires_outside_allowlist(self, fs_workspace):
        """Refusal is unconditional — even for a path outside the allowlist."""
        result = tool_write("/etc/SOUL.md", "x")
        assert "operator-owned" in result

    def test_write_allows_non_locked_md(self, fs_workspace):
        """A different .md (e.g. MEMORY.md, USER.md) is writable."""
        for name in ("MEMORY.md", "USER.md", "NOTES.md"):
            result = tool_write(str(fs_workspace / name), "content")
            assert "Written" in result

    def test_edit_allows_non_locked_md(self, fs_workspace):
        target = fs_workspace / "MEMORY.md"
        target.write_text("old text\n")
        result = tool_edit(str(target), "old", "new")
        assert "Edited" in result
