...
def _attach_metering(daemon, pool):
    ...
    daemon.metering_db = MeteringDB(pool)
    daemon.pool = pool
...
class TestResolveIntegration:
    ...
    @pytest.mark.asyncio
    async def test_text_suppressed_when_tool_calls_coexist(
        self, daemon_with_mock_provider,
    ):
        """When the final agentic response contains both `text` and
        `tool_calls`, the text is preamble/reasoning by Anthropic + OpenAI
        convention — not a user-facing reply.  Pipeline suppresses it
        before reply delivery so the user does not receive third-person
        scaffolding alongside (or instead of) the actual tool-driven
        outcome.

        Regression: 2026-04-29 20:11:24Z, session ded71a54 — model emitted
        text="The AI clips thing — he asked if they helped, she said yes…"
        plus tool_calls=[write], and the third-person paragraph reached
        ...
        # Final response carries text alongside tool_calls — the leak case.
        response = _mock_response(
            text="The AI clips thing — he asked if they helped…",
            tool_calls=[MagicMock(id="tc-1", name="write",
        ...
        assert future.done()
        result = future.result()
        # Text suppressed before delivery — user must not receive the preamble.
        assert result["reply"] == ""
...
class TestBuildHistory:
    ...
    async def test_returns_events(self, tmp_path, pool):
        ...
        daemon.session_mgr = SessionManager(pool)
