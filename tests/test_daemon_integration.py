...
_resolve pattern, _NO_CHANNEL_DELIVERY, process_http_immediate, message loop
HTTP bypass.
...
class TestCheckContextBudget:
    ...
    def test_warning_fires_when_over_40_percent(self, tmp_path, caplog):
        """Warning logged when system prompt exceeds 40% of max_context_tokens."""
        import logging

        # max_context_tokens=1000 → 40% = 400 tokens = 1600 chars.
        # A 2000-char SOUL.md → ~500 tokens → 50% → triggers warning.
        big_soul = "x" * 2000
        daemon = self._make_daemon_with_budget(tmp_path, max_context_tokens=1000, soul_text=big_soul)
    ...
    def test_no_warning_when_under_40_percent(self, tmp_path, caplog):
        """No warning logged when system prompt is within budget."""
        import logging

        # max_context_tokens=100000 → 40% = 40000 tokens = 160000 chars.
    ...
    def test_warning_includes_percentage_and_tokens(self, tmp_path, caplog):
        ...
        big_soul = "x" * 4000  # ~1000 tokens, 100% of a 1000-token budget
        daemon = self._make_daemon_with_budget(tmp_path, max_context_tokens=1000, soul_text=big_soul)
