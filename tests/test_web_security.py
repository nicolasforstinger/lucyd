...
class TestToolWebFetchIntegration:
    ...
    @pytest.mark.asyncio
    async def test_nul_bytes_scrubbed_from_response(self):
        """Tool result must not contain U+0000 — Postgres text cannot store NUL.

        Regression for 2026-05-08 daemon crash: web_fetch returned PDF bytes
        containing literal \\x00 → asyncpg UntranslatableCharacterError on
        persistence → daemon fatal exit mid agentic loop.
        """
        with patch("tools.web.socket.getaddrinfo", return_value=_public_addrinfo(80)), \
             patch("tools.web._safe_opener") as mock_opener:
            mock_resp = MagicMock()
            # Simulate a binary blob (e.g. PDF) leaking through with NUL bytes
            mock_resp.read.return_value = b"%PDF-1.4\x00\x00binary\x00payload"
            mock_resp.headers = {"Content-Type": "application/pdf"}
            mock_opener.open.return_value = mock_resp
            result = await tool_web_fetch("http://example.com/file.pdf")
