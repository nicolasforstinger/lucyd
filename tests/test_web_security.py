...
class TestValidateUrl:
    ...
    def test_blocks_ftp_scheme(self):
        err = _validate_url("ftp://evil.com/payload")
        assert err is not None
        assert "Blocked URL scheme" in err

    def test_blocks_data_scheme(self):
        err = _validate_url("data:text/html,<h1>test</h1>")
        assert err is not None
        assert "Blocked URL scheme" in err
    ...
    def test_blocks_localhost(self):
        err = _validate_url("http://localhost:8080/api")
        assert err is not None

    def test_blocks_127_0_0_1(self):
        err = _validate_url("http://127.0.0.1:8080/api")
        assert err is not None

    def test_blocks_10_network(self):
        err = _validate_url("http://10.0.0.1/secret")
        assert err is not None

    def test_blocks_172_16_network(self):
        err = _validate_url("http://172.16.0.1/internal")
        assert err is not None
    ...
    def test_blocks_unresolvable_hostname(self):
        ...
        err = _validate_url("http://this-domain-does-not-exist-xyzzy.example/")
        assert err is not None
    ...
    def test_blocks_dict_scheme(self):
        err = _validate_url("dict://evil.com/")
        assert err is not None
        assert "Blocked URL scheme" in err
    ...
    def test_dns_error_includes_hostname(self):
        ...
        Kills: f'Cannot resolve hostname: {hostname}' string mutations.
        """
        err = _validate_url("http://this-domain-does-not-exist-xyzzy.example/")
    ...
    def test_private_ip_error_includes_ip_address(self):
        ...
        Kills: f'Blocked: {hostname} resolves to ... {ip_str}' mutations.
        """
        err = _validate_url("http://192.168.1.1/")
    ...
    def test_default_port_443_when_no_port(self):
        ...
        Kills: parsed.port or 443 → 444/0/removed.
        """
    ...
    def test_explicit_port_passthrough(self):
        ...
        with patch("tools.web.socket.getaddrinfo", return_value=fake) as mock_gai:
            result = _validate_url("https://example.com:8080/page")
            assert result is None
    ...
    def test_getaddrinfo_uses_ipproto_tcp(self):
        ...
        Kills: proto=socket.IPPROTO_TCP removal/mutation.
        """
        import socket as _socket
        fake = [(_socket.AF_INET, _socket.SOCK_STREAM, 6, "", ("93.184.216.34", 443))]
        with patch("tools.web.socket.getaddrinfo", return_value=fake) as mock_gai:
...
class TestSafeRedirectHandler:
    ...
    def test_redirect_to_public_url_allowed(self):
        ...
        # Mock DNS to return a public IP so _validate_url passes
