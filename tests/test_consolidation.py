...
class TestExtractEpisode:
    ...
    async def test_valid_episode_stored(self, mem_conn):
        ...
        episode_id, _ = await extract_episode(
    ...
    async def test_trivial_episode_returns_none(self, mem_conn):
        ...
        result, _ = await extract_episode(
            "text", "sess1", provider, [{"text": "persona"}], mem_conn,
        )
...
class TestConsolidateSession:
    ...
    async def test_full_pipeline(self, mem_conn):
        ...
        # Single provider returns fact response first, then episode response
        fact_provider = _make_provider(fact_response)
        episode_provider = _make_provider(episode_response)
        provider = _make_provider("")
        provider.complete = AsyncMock(side_effect=[
            fact_provider.complete.return_value,
            episode_provider.complete.return_value,
        ])
