...
def populated_conn(mem_conn):
    ...
    # Aliases
    mem_conn.execute(
        "INSERT INTO entity_aliases (alias, canonical) VALUES ('alex_johnson', 'alex')"
    )
    mem_conn.execute(
...
class TestResolveEntity:
    def test_alias_resolves(self, populated_conn):
        assert resolve_entity("alex_johnson", populated_conn) == "alex"
...
class TestFormatFactRow:
    """Tests for _format_fact with sqlite3.Row (dict-like access pattern)."""
