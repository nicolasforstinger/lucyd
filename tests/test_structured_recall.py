...
async def populated_pool(pool: Any) -> Any:
    ...
    await pool.execute(
        ...
        "source_session, accessed_at) "
        "VALUES ($1, $2, $3, $4, $5, now())",
        "nicolas", "cat_name", "Miso", 0.9, "test",
    )
    await pool.execute(
        "INSERT INTO knowledge.facts "
        "(entity, attribute, value, confidence, "
    ...
    await pool.execute(
        ...
        "nicolas", "lives_in", "Germany", 0.9, "old",
    )

    # Aliases
    await pool.execute(
        "INSERT INTO knowledge.entity_aliases "
        "(alias, canonical) VALUES ($1, $2)",
        "alex_johnson", "alex",
    )
    await pool.execute(
        "INSERT INTO knowledge.entity_aliases "
    ...
    await pool.execute(
        "INSERT INTO knowledge.entity_aliases "
        "(alias, canonical) VALUES ($1, $2)",
        "uncle_charles", "uncle_charles",
    )
    await pool.execute(
        "INSERT INTO knowledge.entity_aliases "
        "(alias, canonical) VALUES ($1, $2)",
        "charles", "uncle_charles",
    )
...
class TestLookupFacts:
    ...
    async def test_updates_accessed_at(self, populated_pool: Any) -> None:
        ...
        before = await populated_pool.fetchval(
            ...
            "WHERE TRUE "
            "AND entity = $1 AND invalidated_at IS NULL",
            "lucy",
        )
