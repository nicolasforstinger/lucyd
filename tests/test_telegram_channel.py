...
def _make_channel(_token_value="fake-token", **overrides):
    import os
    env_var = f"_TEST_TG_{id(overrides)}"
    os.environ[env_var] = _token_value
    defaults = {
        "token_env": env_var,
        "allow_from": [111, 222],
...
class TestChunkText:
    def test_short_text_single_chunk(self):
        ch = _make_channel(text_chunk_limit=100)
        assert ch._chunk_text("hello") == ["hello"]

    def test_exact_limit_single_chunk(self):
        ch = _make_channel(text_chunk_limit=5)
        assert ch._chunk_text("hello") == ["hello"]
...
class TestInit:
    def test_base_url_from_token(self):
        ch = _make_channel(_token_value="ABC123")
    ...
    def test_chunk_limit_stored(self):
        ch = _make_channel(text_chunk_limit=5000)
    ...
    def test_token_stored(self):
        ch = _make_channel(_token_value="my-secret-token")
...
class TestApi:
    ...
    async def test_url_includes_method_name(self):
        ch = _make_channel(_token_value="tok123")
        mock_client = AsyncMock()
        mock_client.is_closed = False
        mock_client.post.return_value = _mock_response({"ok": True, "result": {}})
        ch._client = mock_client
...
class TestConnectStrong:
    ...
    async def test_connect_calls_getMe_exact_url(self):
        ch = _make_channel(_token_value="tok")
        mock_client = AsyncMock()
        mock_client.is_closed = False
        mock_client.post.return_value = _mock_response({
...
class TestDownloadFileStrong:
    ...
    async def test_download_url_includes_token_and_path(self, tmp_path):
        ch = _make_channel(_token_value="MYTOKEN", download_dir=str(tmp_path))
        mock_client = AsyncMock()
        mock_client.is_closed = False
        mock_client.post.return_value = _mock_response({
            "ok": True, "result": {"file_path": "photos/pic.jpg"},
...
class TestChunkTextStrong:
    ...
    def test_newline_split_exact_content(self):
        ch = _make_channel(text_chunk_limit=7)
        result = ch._chunk_text("aaa\nbbb\nccc")
...
class TestDownloadFileExact:
    ...
    async def test_download_calls_getFile_exact_method(self, tmp_path):
        ...
        ch = _make_channel(_token_value="tok", download_dir=str(tmp_path))
        mock_client = AsyncMock()
        mock_client.is_closed = False
        mock_client.post.return_value = _mock_response({
            "ok": True, "result": {"file_path": "f/test.bin"},
...
class TestExtractMarkdownLinks:
    def test_no_links_unchanged(self):
        ...
        cleaned, links = _extract_markdown_links(text)
        assert cleaned == text
        assert links == []

    def test_single_link(self):
        text = "check the [JP store](https://native-store.net/) for details"
        cleaned, links = _extract_markdown_links(text)
        assert cleaned == "check the JP store [1] for details"
        assert links == [("JP store", "https://native-store.net/")]

    def test_multiple_links_sequential(self):
        text = "[one](https://a.com) and [two](https://b.com) and [three](http://c.com)"
        cleaned, links = _extract_markdown_links(text)
        assert cleaned == "one [1] and two [2] and three [3]"
        assert len(links) == 3
        assert links[0] == ("one", "https://a.com")
        assert links[1] == ("two", "https://b.com")
        assert links[2] == ("three", "http://c.com")

    def test_link_mid_sentence(self):
        text = "visit [the shop](https://shop.com) today"
        cleaned, links = _extract_markdown_links(text)
        assert cleaned == "visit the shop [1] today"
        assert links == [("the shop", "https://shop.com")]

    def test_non_http_links_ignored(self):
        text = "see [file](ftp://example.com/f) or [mail](mailto:x@y.z)"
        cleaned, links = _extract_markdown_links(text)
        assert cleaned == text
        assert links == []

    def test_empty_text(self):
        cleaned, links = _extract_markdown_links("")
        assert cleaned == ""
        assert links == []

    def test_url_with_path_and_query(self):
        text = "[item](https://shop.com/path?q=1&r=2)"
        cleaned, links = _extract_markdown_links(text)
        assert cleaned == "item [1]"
        assert links == [("item", "https://shop.com/path?q=1&r=2")]
...
class TestBuildInlineKeyboard:
    ...
    def test_single_link(self):
        result = _build_inline_keyboard([("label", "https://a.com")])
        assert result == {"inline_keyboard": [[{"text": "1", "url": "https://a.com"}]]}

    def test_four_links_one_row(self):
        links = [(f"l{i}", f"https://{i}.com") for i in range(1, 5)]
        result = _build_inline_keyboard(links)
        assert result is not None
        rows = result["inline_keyboard"]
        assert len(rows) == 1
        assert len(rows[0]) == 4
        assert rows[0][0] == {"text": "1", "url": "https://1.com"}
        assert rows[0][3] == {"text": "4", "url": "https://4.com"}

    def test_five_links_two_rows(self):
        links = [(f"l{i}", f"https://{i}.com") for i in range(1, 6)]
        result = _build_inline_keyboard(links)
        assert result is not None
        rows = result["inline_keyboard"]
        assert len(rows) == 2
        assert len(rows[0]) == 4
        assert len(rows[1]) == 1
        assert rows[1][0] == {"text": "5", "url": "https://5.com"}

    def test_button_text_is_number(self):
        links = [("JP store", "https://a.com"), ("US store", "https://b.com")]
        result = _build_inline_keyboard(links)
        assert result is not None
...
class TestSendWithLinks:
    @pytest.mark.asyncio
    async def test_text_with_links_sends_reply_markup(self):
        ch = _make_channel()
        mock_client = AsyncMock()
        mock_client.is_closed = False
        mock_client.post.return_value = _mock_response({"ok": True, "result": {}})
        ...
        await ch.send("Nicolas", "see [link](https://example.com)")

        call_json = mock_client.post.call_args[1]["json"]
        assert call_json["text"] == "see link [1]"
        assert "reply_markup" in call_json
        kb = call_json["reply_markup"]["inline_keyboard"]
        assert kb == [[{"text": "1", "url": "https://example.com"}]]

    @pytest.mark.asyncio
    async def test_text_without_links_no_reply_markup(self):
        ch = _make_channel()
        mock_client = AsyncMock()
        mock_client.is_closed = False
        mock_client.post.return_value = _mock_response({"ok": True, "result": {}})
        ...
        await ch.send("Nicolas", "plain text no links")

        call_json = mock_client.post.call_args[1]["json"]
        assert call_json["text"] == "plain text no links"
        assert "reply_markup" not in call_json

    @pytest.mark.asyncio
    async def test_chunked_text_buttons_on_last_chunk(self):
        ...
        # Text with link that will chunk (line breaks for clean split)
        text = "first line here\nsecond [link](https://example.com)"
        await ch.send("Nicolas", text)

        calls = mock_client.post.call_args_list
        assert len(calls) >= 2
        # Only the last call should have reply_markup
        for call in calls[:-1]:
            assert "reply_markup" not in call[1]["json"]
        assert "reply_markup" in calls[-1][1]["json"]

    @pytest.mark.asyncio
    async def test_text_cleaned_of_markdown_syntax(self):
        ch = _make_channel()
        mock_client = AsyncMock()
        mock_client.is_closed = False
        mock_client.post.return_value = _mock_response({"ok": True, "result": {}})
        ...
        await ch.send("Nicolas", "[A](https://a.com) and [B](https://b.com)")

        call_json = mock_client.post.call_args[1]["json"]
        assert "[A](" not in call_json["text"]
        assert "[B](" not in call_json["text"]
        assert "A [1] and B [2]" == call_json["text"]

    @pytest.mark.asyncio
    async def test_attachment_fallback_text_with_links(self, tmp_path):
        """When attachment text is sent separately, links are still extracted."""
        ch = _make_channel()
        mock_client = AsyncMock()
        mock_client.is_closed = False
        mock_client.post.return_value = _mock_response({"ok": True, "result": {}})
        ...
        # Audio attachment — text won't be consumed as caption
        audio_file = tmp_path / "voice.ogg"
        audio_file.write_bytes(b"audiodata")

        await ch.send("Nicolas", "see [link](https://example.com)", attachments=[str(audio_file)])

        # Last call should be sendMessage with the text + reply_markup
        last_call = mock_client.post.call_args_list[-1]
        assert "sendMessage" in last_call[0][0]
        call_json = last_call[1]["json"]
        assert call_json["text"] == "see link [1]"
        assert "reply_markup" in call_json
...
def _mg_update(update_id, message_id, user_id, media_group_id, *,
               caption="", has_photo=True, text=""):
    """Build a Telegram update dict for a media-group message."""
    msg: dict = {
        "from": {"id": user_id, "username": "nico"},
        "chat": {"id": user_id},
    ...
    if text:
        msg["text"] = text
    if has_photo:
        msg["photo"] = [{"file_id": f"photo_{message_id}", "file_size": 100}]
    return {"update_id": update_id, "message": msg}


class TestMediaGroupBatching:
    """Media groups (albums) should be merged into a single InboundMessage."""

    @pytest.mark.asyncio
    async def test_media_group_merged_into_single_message(self, tmp_path):
        """Three photos with the same media_group_id yield one InboundMessage."""
        ch = _make_channel(download_dir=str(tmp_path))
        mock_client = AsyncMock()
        mock_client.is_closed = False
        ...
        mg_id = "album_123"
        call_count = 0
        # Simulate time advancing past the flush delay between polls.
        mono_time = [0.0]
        ...
        async def mock_post(url, **kwargs):
            ...
            if "getUpdates" in url:
                mono_time[0] += 1.0  # Advance past _MEDIA_GROUP_DELAY
                if call_count == 1:
                    return _mock_response({
                        "ok": True,
                        "result": [
                            _mg_update(100, 10, 111, mg_id, caption="first pic"),
                            _mg_update(101, 11, 111, mg_id),
                            _mg_update(102, 12, 111, mg_id, caption="third pic"),
                        ],
                    })
                # Second poll (short-timeout flush) — empty, then break.
                if call_count == 2:
            ...
            resp = MagicMock(spec=httpx.Response)
            resp.status_code = 200
            resp.content = b"\xff\xd8fake-jpg"
            resp.raise_for_status = MagicMock()
            return resp

        mock_client.post = mock_post
        mock_client.get = mock_post  # File downloads use GET
        ch._client = mock_client

        messages = []
        with patch("channels.telegram.time") as mock_time:
            mock_time.monotonic = lambda: mono_time[0]
            mock_time.time = time.time  # Preserve for file naming
            with pytest.raises(httpx.ConnectError):
                async for msg in ch._poll_loop():
                    messages.append(msg)
        ...
        assert len(messages) == 1
        merged = messages[0]
        assert merged.sender == "Nicolas"
        assert "first pic" in merged.text
        assert "third pic" in merged.text
        assert merged.attachments is not None
        assert len(merged.attachments) == 3

    @pytest.mark.asyncio
    async def test_non_media_group_messages_pass_through(self):
        """Regular messages (no media_group_id) are yielded immediately."""
        ch = _make_channel()
        mock_client = AsyncMock()
        mock_client.is_closed = False
        call_count = 0
        ...
        assert len(messages) == 1
        assert messages[0].text == "standalone"

    @pytest.mark.asyncio
    async def test_media_group_preserves_album_order(self, tmp_path):
        """Attachments are ordered by message_id even if updates arrive shuffled."""
        ch = _make_channel(download_dir=str(tmp_path))
        mock_client = AsyncMock()
        mock_client.is_closed = False
        ...
        async def mock_post(url, **kwargs):
            ...
            if "getUpdates" in url:
                mono_time[0] += 1.0
                if call_count == 1:
                    # Send updates in reverse order — merge should sort by message_id.
                    return _mock_response({
                        "ok": True,
                        "result": [
                            _mg_update(102, 30, 111, mg_id, caption="C"),
                            _mg_update(100, 10, 111, mg_id, caption="A"),
                            _mg_update(101, 20, 111, mg_id, caption="B"),
                        ],
                    })
                if call_count == 2:
                    return _mock_response({"ok": True, "result": []})
        ...
        assert len(messages) == 1
        # Captions should be in message_id order: A, B, C
        assert messages[0].text == "A\nB\nC"

    @pytest.mark.asyncio
    async def test_media_group_from_disallowed_user_skipped(self):
        """Media group from a user not in allow_from is dropped."""
        ch = _make_channel()
        mock_client = AsyncMock()
        mock_client.is_closed = False
        call_count = 0
            ...
            if "getUpdates" in url:
                ...
                if call_count == 1:
                    return _mock_response({
                        "ok": True,
                        "result": [
                            _mg_update(100, 10, 999, "album_x"),  # user 999 not allowed
                            _mg_update(101, 11, 999, "album_x"),
                        ],
                    })
                if call_count == 2:
                    return _mock_response({"ok": True, "result": []})
    ...
    @pytest.mark.asyncio
    async def test_media_group_with_reply_extracts_quote(self, tmp_path):
        """Quote context from the first message in a media group is preserved."""
        ch = _make_channel(download_dir=str(tmp_path))
        mock_client = AsyncMock()
        mock_client.is_closed = False
        call_count = 0
        ...
        async def mock_post(url, **kwargs):
            ...
            if "getUpdates" in url:
                mono_time[0] += 1.0
                if call_count == 1:
                    msg1 = _mg_update(100, 10, 111, "album_q")
                    # Add reply_to_message on the first message
                    msg1["message"]["reply_to_message"] = {
                        "text": "original message",
                    }
                    return _mock_response({
                        "ok": True,
                        "result": [msg1, _mg_update(101, 11, 111, "album_q")],
                    })
                if call_count == 2:
                    return _mock_response({"ok": True, "result": []})
                raise httpx.ConnectError("stop")
        ...
        assert len(messages) == 1
        assert messages[0].quote == "original message"

    @pytest.mark.asyncio
    async def test_mixed_media_group_and_standalone(self, tmp_path):
        """A batch with both media-group and standalone messages handles both."""
        ch = _make_channel(download_dir=str(tmp_path))
        mock_client = AsyncMock()
        mock_client.is_closed = False
        call_count = 0
                ...
                if call_count == 1:
                    return _mock_response({
                        ...
                        "result": [
                            ...
                            _mg_update(100, 10, 111, "album_mix"),
                            _mg_update(101, 11, 111, "album_mix"),
                        ],
                    })
                if call_count == 2:
                    return _mock_response({"ok": True, "result": []})
        ...
        # Standalone yielded first (immediately), then merged group after flush.
        assert len(messages) == 2
        assert messages[0].text == "standalone text"
        assert messages[1].attachments is not None
        assert len(messages[1].attachments) == 2

    @pytest.mark.asyncio
    async def test_short_poll_timeout_when_groups_pending(self):
        """When media groups are buffered, poll uses short timeout (1s)."""
        ch = _make_channel()
        mock_client = AsyncMock()
        mock_client.is_closed = False
        call_count = 0
        ...
        async def mock_post(url, **kwargs):
            ...
            if "getUpdates" in url:
                ...
                poll_timeouts.append(body.get("timeout", -1))
                mono_time[0] += 1.0
                if call_count == 1:
                    return _mock_response({
                        "ok": True,
                        "result": [
                            _mg_update(100, 10, 111, "album_t", has_photo=False),
                        ],
                    })
                if call_count == 2:
                    # Second poll should have short timeout
                    return _mock_response({"ok": True, "result": []})
                raise httpx.ConnectError("stop")
        ...
        # First poll: 30s (no pending groups), second: 1s (group buffered)
        assert poll_timeouts[0] == 30
        assert poll_timeouts[1] == 1


class TestExtractQuote:
    """_extract_quote refactored from _parse_message — verify it standalone."""

    def test_no_reply(self):
        assert TelegramChannel._extract_quote({}) is None

    def test_text_reply(self):
        msg = {"reply_to_message": {"text": "hello"}}
        assert TelegramChannel._extract_quote(msg) == "hello"

    def test_caption_reply(self):
        msg = {"reply_to_message": {"caption": "pic caption"}}
        assert TelegramChannel._extract_quote(msg) == "pic caption"

    def test_telegram_quote_preferred(self):
        msg = {
            "reply_to_message": {"text": "full text"},
            "quote": {"text": "selected part"},
        }
        assert TelegramChannel._extract_quote(msg) == "selected part"

    def test_voice_fallback(self):
        msg = {"reply_to_message": {"voice": {"duration": 5}}}
        assert TelegramChannel._extract_quote(msg) == "[voice message]"

    def test_photo_fallback(self):
        msg = {"reply_to_message": {"photo": [{}]}}
        assert TelegramChannel._extract_quote(msg) == "[photo]"

    def test_sticker_with_emoji(self):
        msg = {"reply_to_message": {"sticker": {"emoji": "🦇"}}}
        assert TelegramChannel._extract_quote(msg) == "[sticker 🦇]"

    def test_document_with_name(self):
        msg = {"reply_to_message": {"document": {"file_name": "doc.pdf"}}}
        assert TelegramChannel._extract_quote(msg) == "[document: doc.pdf]"

    def test_audio_fallback(self):
        msg = {"reply_to_message": {"audio": {"duration": 120}}}
        assert TelegramChannel._extract_quote(msg) == "[audio]"
