...
# Ephemeral talkers: their turns get no user-facing delivery and the session
# closes after the reply (user/operator are conversational and persist).
EPHEMERAL_TALKERS: frozenset[Talker] = frozenset(("system", "agent"))
...
_SCHEMA: dict[str, tuple[tuple[str, ...], type, Any]] = {
    ...
    # User's wall-clock timezone. Scheduling tools interpret an absolute
    # `when` in this zone so the model never does UTC/offset math; default
    # UTC keeps it correct on any deploy that doesn't set it.
    "user_timezone":        (("user", "timezone"),                 str,   "UTC"),
    ...
    "log_pii_safe":     (("logging", "pii_safe"),       bool, True),
    # ── Strategy ────────────────────────────────────────────────
    ...
    # ── Bridges (outbound) ─────────────────────────────────────
    "bridges_primary":        (("bridges", "primary"),        str,   ""),
}
...
class Config:
    ...
    def _resolve_data_dir_paths(self) -> None:
        ...
        All paths (state_dir, log_file, http_download_dir)
        ...
        for name in ("state_dir", "log_file"):
