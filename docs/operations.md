...
```bash
sudo cp ~/lucyd/lucyd.service /etc/systemd/system/
...
curl -X POST -H "Authorization: Bearer $TOKEN" \
  -H "Content-Type: application/json" \
  -d '{"event": "email_received", "data": {"from": "someone@example.com"}}' \
...
| `priority` | no | `"normal"` | `"normal"` (operational tier) or `"urgent"` (full tier) |
