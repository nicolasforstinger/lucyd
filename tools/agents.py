...
async def tool_sessions_spawn(
    ...
    try:
        # Late import: the agentic loop is a separate module that also
        # depends on tools (via ToolRegistry). Importing it eagerly here
        # would also make every test that patches `agentic.run_agentic_loop`
        # have to re-target the patch at `tools.agents.run_agentic_loop`.
        import agentic
        loop_cfg = agentic.LoopConfig(
            max_turns=max_turns,
            timeout=timeout,
            api_retries=_config.api_retries,
            api_retry_base_delay=_config.api_retry_base_delay,
        )
        response = await agentic.run_agentic_loop(
            provider=provider,
            system=fmt_system,
            messages=messages,
            tools=scoped_tools,
...
TOOLS: list[ToolSpec] = [
    ToolSpec(
        ...
        description=(
            ...
            "context is discarded after the task, keeping your main session clean. Delegate work "
            "that would pour technical or code-like text into your context: research, web searches, "
            "reading or processing files, comparing options, analysis, drafting or editing documents, "
            "running commands. The sub-agent does the legwork and hands back only a clean summary."
        ),
        input_schema={
            "type": "object",
            "properties": {
