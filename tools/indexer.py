...
# Pool (set by configure, used by index_workspace)
...
async def get_indexed_files(pool: asyncpg.Pool) -> dict[str, str]:
    """Returns {path: content_hash} from search.files table."""
    rows = await pool.fetch("SELECT path, hash FROM search.files")
...
async def update_chunks(
    ...
    async with pool.acquire() as conn:
        async with conn.transaction():
            ...
            await conn.execute(
                "DELETE FROM search.chunks WHERE path = $1",
                path,
            )
...
async def remove_stale_files(
    ...
    rows = await pool.fetch("SELECT path FROM search.files")
    removed = []
    for row in rows:
        ...
        if db_path not in current_paths:
            async with pool.acquire() as conn:
                async with conn.transaction():
                    ...
                    await conn.execute(
                        "DELETE FROM search.files WHERE path = $1",
                        db_path,
                    )
...
async def index_workspace(
    ...
    indexed = await get_indexed_files(pool)
    ...
    for rel_path, abs_path in file_list:
        ...
        await cache_embeddings(pool, chunk_texts, embeddings, model)

    # 4. Remove stale files
    removed = await remove_stale_files(pool, scanned_paths)
    summary["removed"] = removed

    # 5. Final counts (no FTS rebuild needed — tsvector is auto-maintained)
    total_chunks = await pool.fetchval("SELECT COUNT(*) FROM search.chunks")
    total_files = await pool.fetchval("SELECT COUNT(*) FROM search.files")
...
async def get_index_status(
    ...
    total = await pool.fetchval("SELECT COUNT(*) FROM search.chunks")
